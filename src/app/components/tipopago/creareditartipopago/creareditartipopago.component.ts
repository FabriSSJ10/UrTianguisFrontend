...
export class CreareditartipopagoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      hcodigo: [''],
      htipo_Pago: ['', [Validators.required, Validators.maxLength(20)]],
    });
  }

  insertar(): void {
    if (this.form.valid) {
      this.tipopago.id_tipopago = this.form.value.hcodigo;
    ...
    if (this.edicion) {
      this.tS.listId(this.id).subscribe((data) => {
        ...
        this.form.patchValue({
          hcodigo: this.tipopago.id_tipopago,
          htipo_Pago: this.tipopago.tipo_pago
        });
      });
    }
  }
