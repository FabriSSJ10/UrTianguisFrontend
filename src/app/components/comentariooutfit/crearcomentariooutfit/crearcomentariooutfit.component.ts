...
export class CrearcomentariooutfitComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      hid:[''],
      hcalificacion:['',[Validators.required, Validators.min(1), Validators.max(5)]],
      hcomentario:['',[Validators.required, Validators.maxLength(500)]],
  ...
  insertar(): void {
    if (this.form.valid) {
      ...
      this.cO.calificacion=this.form.value.hcalificacion;
    ...
    if (this.edicion) {
      this.comS.listId(this.id).subscribe((data) => {
        this.form = new FormGroup({
          ...
          hou: new FormControl(data.ou.id_outfit),
        });
      });
    }
  }

  imagenOutfitSeleccionada: string | null = null;
  mostrarImagen(idOutfit: number): void {
    const outfitSeleccionado = this.listaoutfit.find(outfit => outfit.id_outfit === idOutfit);
    this.imagenOutfitSeleccionada = outfitSeleccionado ? outfitSeleccionado.imagen : null;
  }
}
