...
export class CreareditarpagoComponent {
  ...
  ngOnInit(): void {
    ...
    this.form=this.formBuilder.group({
      hid:[''],
      hmonto: ['', [Validators.required, Validators.minLength(0)]],
      hfechapago:[new Date(),Validators.required],
      hnumtarjeta: ['', [Validators.minLength(19), Validators.maxLength(19)]],
      hfechavencimiento: ['', [this.validarFechaFutura]],
      hcodvalidacion:['', Validators.maxLength(10)],
      htelefono:['', Validators.maxLength(13)],
    ...
    if (this.edicion) {
      this.pS.listId(this.id).subscribe((data) => {
        this.form = new FormGroup({
          ...
          htipopago: new FormControl(data.tp.id_tipopago),
        });
      });
    }
  }
  validarFechaFutura(control: FormControl) {
  ...
  formatCardNumber(event: any) {
    let value = event.target.value.replace(/\D/g, ''); // Eliminar todo lo que no sea número
    if (value.length > 4) {
      value = value.replace(/(\d{4})(?=\d)/g, '$1 '); // Añadir espacio después de cada 4 dígitos
    }
