...
export class CreareditarnotificacionComponent implements OnInit{
  ...
  listaConfirmarPedido:{value:boolean,viewValue:string}[]=[
    { value: true, viewValue: 'Si'},
    { value: false, viewValue: 'No'},
  ];
  listaConfirmarEntrega:{value:boolean,viewValue:string}[]=[
    { value: true, viewValue: 'Si'},
    { value: false, viewValue: 'No'},
  ];
  constructor(
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      hcodigo: [''],
      hconfirmarpedido: ['', Validators.required],
      hconfirmarentrega: ['', Validators.required],
    });
  }
  insertar():void{
    if(this.form.valid){
      this.notificacion.id_notificacion=this.form.value.hcodigo;
      this.notificacion.confirmacion_pedido=this.form.value.hconfirmarpedido;
      this.notificacion.confirmacion_entrega=this.form.value.hconfirmarentrega;
      if (this.edicion){
    ...
    if(this.edicion){
      this.n.listId(this.id).subscribe((data)=>{
        this.form=new FormGroup({
          ...
          hconfirmarpedido:new FormControl(data.confirmacion_pedido),
          hconfirmarentrega:new FormControl(data.confirmacion_entrega),
          })
      })
    }
  }
